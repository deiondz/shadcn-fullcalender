...
import EventModal, { ColorScheme } from "@shadcn-fullcalender/event-components/eventmodal";
        ...
        <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-4">
            <div className="mb-4 relative z-20">
                <div className="grid grid-cols-8 gap-0">
                    {Array.from({length: 8}).map((_, i) => <div key={`week-view-top-header-${i}`} className="text-center text-xs font-semibold"></div>)}
                </div>
                ...
                <div data-radix-scroll-area-viewport="" className="h-full w-full rounded-[inherit]" style={{overflow: 'hidden scroll'}}>
                    <div style={{minWidth: '100%', display: 'table'}}>
                        <div className="relative mb-2">
                            <div className="grid grid-cols-8 gap-0">
                                <div className="sticky top-0 left-0 z-30 bg-background" type="button" aria-haspopup="dialog" aria-expanded="false" aria-controls="radix-:r3j6:" data-state="closed">
                                ...
                                <div className="col-span-7 relative z-10">
                                    <div className="grid grid-cols-7 gap-0">
                                        {weekDays.map((weekDay, i) => <DayView key={`week-day-${i}`} events={calendarEvents} day={weekDay} />)}
                                        <div className="absolute top-0 right-0 w-px h-full bg-border"></div>
                                    </div>
                                </div>
                            </div>
                            <div className="absolute left-[12.5%] right-0 h-px bg-primary z-40 pointer-events-none" style={{display: 'none'}}>
...
export function AllDayEventView({
    ...
    return (
        <div className="min-h-[24px] px-1">
            <div className={`bg-yellow-200 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-200 px-2 py-1 rounded cursor-pointer transition-colors duration-200 hover:bg-opacity-40 dark:hover:bg-opacity-40 min-h-[24px] text-xs border-x-0 ${isLast ? 'border-r-4 border-r-black/70 dark:border-r-white/50' : ''}`}
                type="button"
    ...
    return (
        <div className="grid grid-cols-8 gap-0 relative">
            <div className="absolute left-0 top-0 bottom-2 w-[12.5%] flex items-center justify-center z-10">
                <div className="text-xs truncate font-semibold whitespace-nowrap">Full-Day Events ({events.length})</div>
            </div>
            <div className="col-span-8 grid grid-cols-8 gap-0 mb-1"
                style={{opacity: 1, willChange: 'opacity', transform: 'none'}}>
                <div className="w-[12.5%]"></div>
                {events.map((event, i) => <AllDayEventView
                    key={`all-day-event-${i}`}
                    event={event}
                    isLast={events.length === i + 1} />
                )}
                {Array.from({length: 7 - events.length}).map((_, i) => <div key={`all-day-event-placeholder-${i}`} className="min-h-[24px] px-1"></div>)}
            </div>
        </div>
    );
}
...
export function DayViewEvent({
    ...
    return (
        <Button 
            ...
            {description && <div className="hidden w-full max-w-md sm:block truncate">
                <p className="truncate text-left font-light">{description}</p></div>}
            <div className="text-xs">{format(from, 'HH:mm')} - {format(to, 'HH:mm')}</div>
        </Button>
    );
}
...
export function DayView({
    ...
    const eventsWithOverlap = dayEvents.map((event, ind) => {
        const overlapingEvents = dayEvents.filter(evt => areIntervalsOverlapping(
    ...
    return (
        <div className="relative">
            <div
                className="sticky top-0 z-20 bg-background h-12 flex items-center justify-center border-b border-border">
                <div className="text-center">
                    <div className="text-xs font-semibold">{dayAsString}</div>
                    <div
                        className={`${isTodaysDate ? 'bg-primary text-primary-foreground' : ''} text-sm rounded-full w-6 h-6 flex items-center justify-center mx-auto`}>
                        {dayInTheMonth}</div>
                </div>
            </div>
            <div className="relative h-[calc(100%-3rem)]">
                {/* Render Hours Cells */}
                {Array.from({length: 24}).map((_, i) =>
                    <div key={`day-${dayInTheMonth}-${i}`}
                        className={`h-16 ${i > 0 ? 'border-t border-border' : ''} ${i == 24-1 ? 'border-b' : ''}`}>
                    </div>
                )}
                {/* Render Events */}
                {eventsWithOverlap.map(({event, overlapingEvents}, i) => <EventModal
                        key={`event-modal-${i}`}
                        trigger={<DayViewEvent
                            key={`day-${dayInTheMonth}-event-${i}`}
                            event={event}
                            width={overlapingEvents.length === 0 ? 100 : Math.round(100 / overlapingEvents.length)}
                            widthOffsetIndex={
                                overlapingEvents.findIndex(({ title, description, from, to}) => title === event.title &&
                                description === event.description && from === event.from && to === event.to
                            )} />}
                        eventdetails={{
                            ...
                            // enddate: '',
                            // endtime: '',
                            colorScheme: (event.category === 'purple' ? 'red' : (
                                event.category === 'yellow' ? 'default' : event.category
                            )) as ColorScheme
                        }}
                    />
                )}
